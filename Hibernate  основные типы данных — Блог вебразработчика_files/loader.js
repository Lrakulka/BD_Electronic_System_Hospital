...
mailru.loader = {
    ...
    _moduleEnv: {}, //additional environment data for the modules
    _loaded: {},
    _readyCallbacks: {},
    ...
    getEnv: function(){
        var isHttps = location.protocol === 'https:',
            ...
            loadFilesHttpsMatch = location.href.match(/__loadHTTPSlocal/i),
            branch = (branchMatch && branchMatch.length > 0) ? branchMatch[1] : '',
            localHttps = Boolean(loadFilesHttpsMatch && loadFilesHttpsMatch.length > 0);
        return { 'branch': branch, 'isHttps': isHttps, 'localHttps': localHttps};
    },
    getMode: function(moduleEnv){
        ...
        // if localHttps is set, we know that the content has to be loaded from 
        // local https reverse proxy (proxied from alpha, branch is set in nginx.conf)
        // localHttps and branch parameters are also persisted in all consequent URL requests
        if (moduleEnv.branch !== '' && moduleEnv.localHttps) {
            mode ='https-local';
        }
        ...
        else if (moduleEnv.branch !== '' && moduleEnv.isHttps) {
            mode ='https-alpha';
        }
        ...
        // if current protocol is https and no branch or localHttps is set, then it's a live site with 
...
(function(){
    ...
    var env = mailru.loader.getEnv(),
        modulePaths = {
            'https-local': 'https://' + env.branch + '.sslmyalpha.imgsmail.ru/mail/ru/images/js/connect/', 
