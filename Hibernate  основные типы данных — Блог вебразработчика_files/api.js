...
mailru.api || (function() {
	...
	api = {
		...
		def: {
            ...
            PROXY_URL: (function(){
                var env = mailru.loader._moduleEnv || { 'branch': '', 'isHttps': false, 'localHttps': false},
                    modulePaths = {
                        'https-local': 'https://' + env.branch + '.sslalpha.my.mail.ru/proxy?', 
                        'https-alpha': 'https://' + env.branch + '.connect.myalpha15.i.mail.ru/proxy?',
                        'http-alpha':  'http://'  + env.branch + '.connect.myalpha15.i.mail.ru/proxy?',
                        'https-live':  'https://connect.mail.ru/proxy?',
                        'http-live':   'http://connect.mail.ru/proxy?'
                    },
                    modulePath = modulePaths[mailru.loader.getMode ? mailru.loader.getMode(env): 'http-live'];
                ...
                if (env.localHttps) {
                    modulePath += '__loadHTTPSlocal=true&'
                }
            ...
            GET_DIALOG_URL: function(dialogName){ 
                var env = mailru.loader._moduleEnv || { 'branch': '', 'isHttps': false, 'localHttps': false},
                    modulePaths = {
                        'https-local': 'https://' + env.branch + '.sslalpha.my.mail.ru/api/' + dialogName + '?',
                        'https-alpha': 'https://' + env.branch + '.my.myalpha15.i.mail.ru/cgi-bin/connect/api/' + dialogName + '?',
                        'http-alpha':  'http://' + env.branch + '.my.myalpha15.i.mail.ru/cgi-bin/connect/api/' + dialogName + '?',
                        'https-live':  'https://my.mail.ru/cgi-bin/connect/api/' + dialogName + '?',
                        'http-live':   'http://my.mail.ru/cgi-bin/connect/api/' + dialogName + '?'
                    },
                    modulePath = modulePaths[mailru.loader.getMode ? mailru.loader.getMode(env): 'http-live'];
            ...
            payments: {
                showDialog: function(params) {
                    ...
                    if (mailru.isApp) {
                        ...
                        mailru.connect.activePaymentWindow = mailru.utils.modal.open(mailru.def.GET_DIALOG_URL('payment_dialog'), params);
                    }
                }
            },
            gift : {
            	send : function (gift_id, text, friend_id) {
                    ...
                    if (mailru.isApp) {
                        ...
                        mailru.intercom.wrp.request('showGiftSendDialog', params);
                    } else {
                        ...
                        var wid = mailru.utils.modal.open(mailru.def.DIALOG_URL + 'friends_select?', params);
                    }
		...
		app: {
            ...
            friends: {
                invite: function () {
                    ...
                    params.sig = mailru.utils.sign(params).sig;

                    mailru.intercom.wrp.request('showInviteFriendsDialog', params);
		...
		connect: {
			 ...
			 */
			_appAutoInstall: function(app_id, callback, session, sessionstr) {
				mailru.utils.requestOverProxy({
					...
					cbid: mailru.callbacks.add(function(data) {
						...
						if (data.length > 1 && typeof data[2] !== 'string' && 'success' in data[2]) {
						...
						// if required data was not obtained from autoInstall, just call the callback with current session/sessionstr data
						else {
							if (typeof callback == 'function') {
								callback(session, sessionstr);
							}
			...
			getLoginStatus: function(callback) {
				mailru.utils.requestOverProxy({
					...
					cbid: mailru.callbacks.add(function(session, sessionstr) {
						...
						if ('connectPartner' in parsedResult && parsedResult['connectPartner'] > 0 && typeof callback != 'undefined') {
							...
							mailru.connect._appAutoInstall(mailru.app_id, callback, session, sessionstr);
						} 
						// and if it was just a normal app without connectPartner=1, dispatch resulting session and fire the callback
							...
							mailru.connect._dispatchSession(session, sessionstr);
							if (typeof callback == 'function') callback(session, sessionstr);
						}
...
// alert(sessionstr);
                ...
                // @todo: I couldn’t find cases where is_app is passed to _dispatchSession, confirm if it's used anywhere at all
				if (session.is_app_user == 0 && !is_app) {
					mailru.connect._clearSession();
				} 
				// and if the app is authorised for the user, request further periodical session prologation
				else {
			 ...
			 * @todo: session argument is not used at all, confirm that it can be deleted
			 * @private
			 */
			_prolongSession: function(session) {
				window.clearTimeout(mailru.connect._prolongSessionTmr);
		...
		plugin: {
			...
			insert: function(element) {
				...
				switch (element.type) {
                    ...
                    case 'recommendations': element.style.display = 'none'; return false; break;
                    case 'groups_widget': url = mailru.def.PLUGIN_URL + 'groups_widget?'; break;
				}
				var wid = 0;
